 ...
 * every other piece (the keychain-snapshot/restore-on-failure fix, the
 * pin-reconciliation logic, the log-only vs enforce split) is unchanged
 * in shape.
 *
 ...
 * from /dev/otp_firewall) and handle_redeliver_packet() (a packet
 * reconstructed from an ack-port REDELIVER, see ack.h) - both need
 * exactly the same trial-decrypt/pin/ack-send logic. On OTP_FW_OK, also
 * sends the delivery ack back to `src_ip` - the receiving half of the
 * same mechanism the egress branch's ack_mark_outstanding() call is the
 * sending half of. */
