/*
 * packet_codec.c - OpenBSD port of firewall/linux-kernel-module/packet_codec.c.
 *
 * UNVERIFIED (not built on a real OpenBSD machine), but relies on the
 * most stable part of the whole port: <netinet/ip.h>/<netinet/tcp.h>/
 * <netinet/udp.h>/<netinet/ip6.h> are the reference BSD sockets
 * networking headers - the same struct ip/ip_hl/ip_p,
 * struct tcphdr/th_sport/th_off, struct udphdr/uh_sport/uh_ulen,
 * struct ip6_hdr/ip6_nxt/ip6_plen field names every BSD (OpenBSD
 * included - pf itself, elsewhere in this port, is OpenBSD's own code)
 * derives from, decades-stable and unchanged across the family. Unlike
 * ../freebsd-kernel-module/packet_codec.c (which had to lean on glibc's
 * optional BSD-compat structs to even compile and test on this project's
 * Linux build machine), this file has nothing to emulate: OpenBSD's own
 * headers use these exact struct/field names natively, needing no
 * compatibility shims at all on the real target - a reason for slightly
 * *more* confidence these names are right, not less, even though (same
 * as the FreeBSD port) this file was only ever compiled here against
 * glibc's own BSD-compat shim, never a real OpenBSD
 * netinet header. Every function body is identical to the FreeBSD/macOS ports'
 * copies of this file, byte-for-byte except this comment and the include
 * list - only fix_ipv{4,6}_lengths_and_checksums() ever touch these BSD
 * struct fields directly; everything else works through them.
 *
 ...
 * file instead of firewall/linux-kernel-module/packet_codec.c.
 */
