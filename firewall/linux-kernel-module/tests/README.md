...
in `../*.c` (i.e. `firewall/linux-kernel-module/*.c`), linking the same unmodified core library
...
`otp_firewalld.c` (the NFQUEUE event loop itself) isn't covered here: it's a thin
...
section for the real insmod/`otp_firewalld`/kill-switch steps instead.
